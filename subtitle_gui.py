...
def is_our_srt(srt_path: Path) -> bool:
    ...
    try:
        first = srt_path.read_text(encoding="utf-8-sig", errors="ignore")[:100]
        return OUR_TAG in first
    except Exception:
        return False
...
def scan_video_status(video: Path, lang: str,
    ...
    Status values:
      NEW          — never seen before, no subtitle
      UNPROCESSED  — seen before but still no subtitle
      DONE         — our subtitle exists, file unchanged
      CHANGED      — file changed since we processed it
      EXTERNAL     — subtitle exists but not ours
    """
    ...
    for srt in candidates:
        if srt.exists():
            ours = is_our_srt(srt)
            if ours:
                # Check if video fingerprint changed since last scan
    ...
    # No subtitle — was it seen before?
...
class SubtitleApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # Library button + queue status
        lib_btn_row = tk.Frame(top_card, bg=BG_PANEL)
        lib_btn_row.pack(fill="x", pady=(0, 8))
        FlatButton(lib_btn_row, "📂  Open Library Manager",
                   command=self._open_library, accent=False).pack(side="left")
        self.lib_status_label = tk.Label(lib_btn_row,
                                         text="No files queued from library.",
                                         font=("Consolas", 9), fg=TEXT_MUTED,
                                         bg=BG_PANEL)
        self.lib_status_label.pack(side="left", padx=(12, 0))

        # Manual queue
        add_row = tk.Frame(top_card, bg=BG_PANEL)
        add_row.pack(fill="x", pady=(0, 6))
        tk.Label(add_row, text="Manual:", font=("Consolas", 9), fg=TEXT_DIM,
                 bg=BG_PANEL).pack(side="left", padx=(0, 6))
        self.add_entry = styled_entry(add_row, self.folder_var, width=44)
        self.add_entry.pack(side="left", fill="x", expand=True, padx=(0, 8))
        FlatButton(add_row, "Browse", command=self._browse).pack(side="left")
        FlatButton(add_row, "Add", command=self._add_to_queue).pack(side="left", padx=(6, 0))

        list_frame = tk.Frame(top_card, bg=BG_PANEL)
        list_frame.pack(fill="x", pady=(0, 6))
        man_scroll = tk.Scrollbar(list_frame, orient="vertical",
        ...
        self.queue_listbox = tk.Listbox(
            ...
            highlightthickness=1, highlightbackground=BORDER,
            activestyle="none", yscrollcommand=man_scroll.set)
        man_scroll.config(command=self.queue_listbox.yview)
        self.queue_listbox.pack(side="left", fill="x", expand=True)
        man_scroll.pack(side="right", fill="y")

        ctrl_row = tk.Frame(top_card, bg=BG_PANEL)
        ctrl_row.pack(fill="x")
        FlatButton(ctrl_row, "▲ Up",     command=self._queue_up).pack(side="left", padx=(0,6))
        FlatButton(ctrl_row, "▼ Down",   command=self._queue_down).pack(side="left", padx=(0,6))
        FlatButton(ctrl_row, "✕ Remove", command=self._queue_remove).pack(side="left", padx=(0,6))
        FlatButton(ctrl_row, "Clear",    command=self._queue_clear).pack(side="left", padx=(0,6))
        FlatButton(ctrl_row, "Scan", command=self._scan_queue).pack(side="right")

        self.file_label = tk.Label(top_card,
                                   text="Add folders and click Scan, or use Library Manager.",
                                   font=("Consolas", 9), fg=TEXT_MUTED,
                                   bg=BG_PANEL, anchor="w")
        self.file_label.pack(fill="x", pady=(6, 0))
    ...
    def _queue_from_library(self, video_files: list):
        ...
        self.lib_status_label.configure(
            text=f"✓  {count} file(s) queued from library.",
            fg=ACCENT)
        self._log(f"✓  {count} file(s) queued from Library Manager.", "ok")
    ...
    def _start(self):
        if self.is_running:
            return
        if not self.queue_folders and not self.video_files:
            self._log("⚠  Add folders to the queue or use Library Manager.", "warn")
            return
        if self.queue_folders:
            self._scan_queue()
    ...
    def _populate(self):
        ...
        for e in self.lib_entries:
            ...
            for ep in e.get("episodes", []):
                ...
                srt_info = "  our srt" if ep["our"] else ("  ext srt" if ep["srt"] else "")
                self.tree.insert(parent, "end",
                                  text=f"  {ep_name}",
                                  values=(ep_badge, "", "", ep["path"] + srt_info),
                                  tags=(ep["status"],))
            shown += 1
